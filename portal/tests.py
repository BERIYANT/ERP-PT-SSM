...
class ContractSmoke(TestCase):
 ...
 def test_every_template_compiles_and_renders_authenticated(self):
  ...
  for file in templates:
   ...
   with self.subTest(template=name):
    ...
    template.render({'request':None,'current_user':self.u.to_dict(),'project_id':self.project.id,'dashboard_project_id':self.project.id})
 ...
 def test_legacy_html_aliases(self):
  ...
  for url in urls:
   with self.subTest(url=url):self.assertEqual(self.c.get(url).status_code,200)
