...
function setActiveSubmenu(element, submenuId) {
...
async function loadProfile() {
    ...
    const u = data.data;
    profileInitial = { nama: u.nama || '', email: u.email || '', phone: u.phone || '', address: '' };
    ...
    if (pRole) pRole.textContent = 'Supervisi';
}
...
async function saveProfile() {
    ...
    if (!nama || !email || !phone || !address) return openModal('warningRequiredModal');
    ...
    const data = await res.json();
    if (!data.success) return alert(data.message || 'Gagal menyimpan profil.');
...
async function changePassword() {
    ...
    if (!old_password || !new_password || !confirm_password) return openModal('warningRequiredModal');
    if (new_password.length < 8) return openModal('warningPasswordLengthModal');
    if (new_password !== confirm_password) return openModal('warningPasswordMatchModal');
    ...
    const data = await res.json();
    if (!data.success) return alert(data.message || 'Gagal mengganti password.');
...
function renderActivity(items, append = false) {
    ...
    <div class="timeline-item">
      <div class="timeline-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><path d="M12 6v6l4 2"></path></svg></div>
    ...
    else timeline.innerHTML = html || '<div class="timeline-item"><div class="timeline-content"><div class="timeline-title">Belum ada aktivitas.</div></div></div>';
}
